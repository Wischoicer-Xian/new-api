...
func TestSyncImageAdaptorGuard(t *testing.T) {
	// Async-only image apiType -> fail-close, clear, non-500, skip-retry.
	err := syncImageAdaptorGuard(constant.APITypeApiNebula, "gpt-image-2")
	...
	assert.Nil(t, syncImageAdaptorGuard(constant.APITypeOpenAI, "gpt-image-1"))
	assert.Nil(t, syncImageAdaptorGuard(constant.APITypeAnthropic, "claude-3"),
