...
func saturateQuota(value float64, op string) (int, *QuotaClamp) {
	return saturateQuotaBounded(value, op, MaxQuota, MinQuota, true)
}

func saturateQuotaBounded(value float64, op string, maxQuota int, minQuota int, inclusive bool) (int, *QuotaClamp) {
	var clamp *QuotaClamp
	switch {
	case math.IsNaN(value):
		clamp = &QuotaClamp{Op: op, Kind: QuotaClampNaN, Original: value, Clamped: 0}
	case (inclusive && value >= float64(maxQuota)) || (!inclusive && value > float64(maxQuota)):
		clamp = &QuotaClamp{Op: op, Kind: QuotaClampOverflow, Original: value, Clamped: maxQuota}
	case (inclusive && value <= float64(minQuota)) || (!inclusive && value < float64(minQuota)):
...
// Use this converter for single-request charges. Positive wallet/top-up entry
// points must use WalletQuotaFromDecimalStrict so they retain the wider
// JavaScript-safe wallet domain while still failing on conversion overflow.
...
func WalletQuotaFromDecimalStrict(d decimal.Decimal) (int, error) {
	f, _ := d.Round(0).Float64()
	return strictQuota(saturateQuotaBounded(f, "WalletQuotaFromDecimal", MaxWalletQuota, -MaxWalletQuota, false))
}
