...
func rejectInvalidCreditedQuota(c *gin.Context, _ int, quota decimal.Decimal) bool {
	// The fork's paid-settlement path may credit an account above the legacy
	// int32 wallet boundary (CreditPaidTopUpTx falls back to the int64 storage
	// cap). Validate only the single charge here; rejecting on current balance
	// would make a customer pay for a top-up that the fork is designed to honor.
...
func rejectInvalidTopUpQuota(c *gin.Context, _ int, amount int64) bool {
	// Keep the upstream per-payment representability check, but leave wallet
	// capacity to CreditPaidTopUpTx so fork paid-top-up semantics are preserved.
...
func EpayNotify(c *gin.Context) {
	...
	// 进程内互斥，单实例串行化重复通知；跨进程由 DB 行锁保证（CompleteEpayTopUpTx 内
	// lockForUpdate）。保留进程内锁可减少同进程并发重复进入业务逻辑，DB 行锁兜底跨实例。
	LockOrder(verifyInfo.ServiceTradeNo)
	defer UnlockOrder(verifyInfo.ServiceTradeNo)

	// 事务外读 topUp 仅用于计算 quotaToAdd 与日志字段；事务内 CompleteEpayTopUpTx 会
	// 权威地重新锁定并校验 provider/status。Amount 创建后不可变，事务外读到旧值不影响
	// 资金正确性。
	...
	if credited {
		...
		// 幂等命中：订单已是 SUCCESS（通常为 epay 重复通知），不再增 quota 也不重复记日志。
