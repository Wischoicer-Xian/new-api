...
func TestRequestAmountAllowsPaidTopUpAboveLegacyWalletBoundary(t *testing.T) {
	oldQuotaPerUnit := common.QuotaPerUnit
	oldDisplayType := operation_setting.GetGeneralSetting().QuotaDisplayType
	oldDB := model.DB
	common.QuotaPerUnit = 500000
	...
	require.NoError(t, model.DB.Create(&model.User{
		...
		// The fork stores balances as int64 and allows paid top-ups to
		// preserve funds after the legacy single-charge int32 boundary.
		Quota:  common.MaxQuota + 1_000_000,
