...
func channelMatchesFilter(ch *Channel, modelName string, filter dto.ChannelFilter) bool {
	...
	case dto.FilterRequestPath:
		...
		// Keep the fork's operation-granular guard on the memory-cache path as
		// well as the DB path: async-only image providers must never be selected
		// for synchronous image relay requests.
		if IsSyncImagePath(filter.RequestPath) && excludeChannelForSyncImage(filter.RequestPath, ch.Type) {
			return false
		}
