...
// legacyAdminOnlyLogOtherKeys retain the fork's established top-level shape for
// model mapping. They remain in stored/admin projections for the existing UI,
// but must be removed from user-facing projections.
var legacyAdminOnlyLogOtherKeys = []string{
	"is_model_mapped",
	"upstream_model_name",
}
...
func formatLogOtherJSON(value string, visibility logOtherVisibility) string {
	...
	if visibility == logOtherVisibilityUser {
		...
		for _, key := range legacyAdminOnlyLogOtherKeys {
			if _, exists := values[key]; exists {
				delete(values, key)
				changed = true
			}
